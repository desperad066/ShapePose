...
class Model(nn.Module):
    def __init__(self, filename_obj, args):
        ...
        self.renderer = sr.SoftRenderer(image_size=args.image_size, sigma_val=1e-3, gamma_val=1e-6, 
    ...
    def predict_multiview(self, image_a, image_b, viewpoint_a, viewpoint_b):
        ...
        # batch_size = image_a.size(0)
        # self.renderer.transform.set_eyes(viewpoint_a)

        # vertices, faces = self.reconstruct(image_a)
        # laplacian_loss = self.laplacian_loss(vertices)
        # flatten_loss = self.flatten_loss(vertices)
    ...
    def predict_latent_pose(self, latentvector, viewpoint_a, viewpoint_b):

        # latentvector.size()[0] == 512
        latentvector = torch.cat((latentvector, latentvector), dim=0)
        # Ma Mb
        vertices, faces = self.decoder(latentvector)
        if self.first_forward:
            print('latentvector_size', latentvector.size(0))
            print('vertices', vertices.shape)
            print('faces', faces.shape)
            self.first_forward = False
        ...
        # if the viewpoints have two viewpoints 0, 1
        # self.renderer.transform.set_eyes(viewpoints[0])
        # silhouettes = self.renderer(vertices, faces)
        ...
        return silhouettes.chunk(4, dim=0), laplacian_loss, flatten_loss, \
             vertices.chunk(4, dim=0), faces.chunk(4, dim=0)

    def predict_image(self, images, viewpoints):
        latentvector = self.encoder(images)
        vertices, faces = self.decoder(latentvector)

        self.renderer.transform.set_eyes(viewpoints)
        silhouettes = self.renderer(vertices, faces)

        return vertices, faces, latentvector, silhouettes

    def forward(self, images=None, viewpoints=None, voxels=None, latentvector=None, task='train'):
        if task == 'train':
            return self.predict_multiview(images[0], images[1], viewpoints[0], viewpoints[1])
        elif task == 'test':
            return self.evaluate_iou(images, voxels)
        elif task == 'use':
            return self.predict_latent_pose(latentvector, viewpoints[0], viewpoints[1])
        elif task == 'use_test':
            return self.predict_image(images,viewpoints)
